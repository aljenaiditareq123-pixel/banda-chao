...
|-------------|--------|----------------|-------|
| **1. Panda Haggle** | ✅ **INSTALLED** | `components/product/PandaHaggleModal.tsx`, `app/[locale]/products/[id]/page-client.tsx` (Haggle button & modal integration) | Complete: Modal component with negotiation chat, price input, counter-offer logic, cart integration. **VERIFY:** Backend API endpoint (`server/src/api/haggle.ts`) |
| **2. Daily Feng Shui** | ✅ **INSTALLED** | `components/home/DailyFengShui.tsx`, `components/home/HomePageClient.tsx` (integration), `getLuckyProducts()` function | Complete: Widget component, luck calculation, product filtering by color, homepage integration |
| **3. Night Market** | ⚠️ **PARTIAL** | `app/globals.css` (CSS theme), `components/nightmarket/NightMarketBanner.tsx`, `contexts/NightMarketContext.tsx`, `hooks/useNightMarket.ts` | CSS and components exist. **VERIFY:** Time detection logic in `useNightMarket.ts`, theme application in layout |
| **4. Clan Buying** | ✅ **INSTALLED** | `components/product/ViralShareModal.tsx`, `components/product/ClanBuyModal.tsx`, `server/src/api/clanBuy.ts`, `server/src/services/viralService.ts`, `app/[locale]/products/[id]/page-client.tsx` | Complete: API, service, UI modals, product page integration with "Buy Alone" and "Start Clan" buttons |
| **5. Banda Pet** | ✅ **INSTALLED** | `components/pet/BandaPet.tsx`, `components/pet/PetFeedModal.tsx`, `server/src/api/pet.ts`, `prisma/schema.prisma` (pet_states, pet_feed_history) | Complete: Component, API, database schema, feeding logic, discount code rewards |
| **6. Blind Box** | ✅ **INSTALLED** | `server/src/api/blindBox.ts`, `components/blindbox/BlindBoxReveal.tsx`, `prisma/schema.prisma` (mystery_lists, is_blind_box fields) | Complete: API, reveal component, schema. **VERIFY:** Integration in order success page |
| **7. Reverse Auction (Flash Drop)** | ✅ **INSTALLED** | `app/[locale]/flash-drop/page-client.tsx`, `app/[locale]/flash-drop/page.tsx`, `server/src/api/flashDrop.ts`, `prisma/schema.prisma` (flash_drops, flash_drop_participants) | Complete: Page, API, timer logic (10s intervals), price freeze, concurrency handling, database schema |
| **8. Video Reviews** | ✅ **INSTALLED** | `components/social/CommentForm.tsx` (video upload), `components/product/VideoReviewsCarousel.tsx`, `prisma/schema.prisma` (review_video_url), `server/src/api/comments.ts` | Complete: Video upload UI, validation (15s/10MB), video player, Stories-style carousel component |
| **9. Virtual Try-On** | ✅ **INSTALLED** | `components/product/VirtualTryOn.tsx` (473 lines), `app/[locale]/products/[id]/page-client.tsx` (Try On button for Fashion category) | Complete: Webcam access, product image overlay, drag/resize, snap photo, canvas capture, Fashion category check |
| **10. Search by Image** | ⚠️ **PARTIAL** | `components/search/SmartSearchBar.tsx` | Search bar component exists with text search. **MISSING:** Camera icon inside search bar, image file picker, "Scanning..." animation, mock similar products display logic |
