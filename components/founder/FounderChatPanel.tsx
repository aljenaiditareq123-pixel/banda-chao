...
export default function FounderChatPanel({ assistantId, currentMode: externalMode }: FounderChatPanelProps) {
  ...
  if (!assistant) {
    return (
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 flex items-center justify-center min-h-[600px]">
        <div className="text-center">
          <span className="text-6xl mb-4 block" aria-hidden="true">🐼</span>
          <p className="text-slate-600">مساعد غير صالح</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm h-full flex flex-col min-h-[600px]">
      {/* Messages Area - Luxury Gulf Founder Style */}
      <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50">
        {messages.map((message) => (
          <div
            key={message.id}
            className={`flex ${message.role === 'founder' ? 'justify-end rtl:justify-start' : 'justify-start rtl:justify-end'}`}
          >
            <div
              className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                message.role === 'founder'
                  ? 'bg-amber-500/10 border border-amber-500/40 text-slate-900'
          ...
          <div className="flex justify-start rtl:justify-end">
            <div className="max-w-[80%] rounded-2xl px-4 py-3 bg-white border border-slate-200">
              <div className="flex items-center gap-2 rtl:flex-row-reverse">
                ...
                <p className="text-sm text-slate-600">{assistant.loadingText}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="p-6 border-t border-slate-200 bg-white rounded-b-3xl">
        <div className="relative">
          <textarea
            ...
            className="w-full min-h-[100px] resize-none rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 pr-14 text-sm text-slate-900 focus:border-amber-500/60 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-colors"
          ...
          {recognitionRef.current && (
            <button
              ...
              className={`absolute left-3 top-3 rtl:left-auto rtl:right-3 p-2 rounded-lg transition ${
                isListening
        ...
        <div className="flex items-center justify-between mt-4 rtl:flex-row-reverse">
          <p className="text-xs text-slate-500">
            سجّل قراراتك أو خططك هنا لتبقى واضحة في رحلة المؤسس.
          </p>
          <button
            type="submit"
            className="bg-slate-900 hover:bg-slate-800 border border-amber-500/40 text-amber-400 px-6 py-2 rounded-xl font-semibold hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
